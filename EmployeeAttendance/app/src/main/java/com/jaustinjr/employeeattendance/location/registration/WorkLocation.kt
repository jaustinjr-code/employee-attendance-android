 ...
 *
 * [Serializable] so the registration layer can persist the registered list as JSON (see
 * [WorkLocationLocalDataSource]); the [init] validation below still runs on deserialize, so a
 * corrupt/tampered store can never yield an invalid location.
 *
