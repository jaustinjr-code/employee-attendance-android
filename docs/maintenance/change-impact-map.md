...
| `statusupdate/StatusUpdateRepository.kt` `StoredStatusUpdate` | the on-disk shape; keeps nullable legacy `didToday` / `plannedTomorrow` / `couldNotDo` for reading updates saved by earlier releases (used only when `answers` is empty, never written), so do not remove them without a migration; unknown question ids are ignored; `ignoreUnknownKeys` must stay on. A decode failure empties all history | `StoredStatusUpdateTest`, `SharedPrefsStatusUpdateLocalDataSourceTest` |
| `statusupdate/StatusUpdateQuestion.kt` `id` | persisted as the key of stored answers: never rename or reuse an id; add a new entry (with strings) to add a question; unknown ids are ignored on load. Declaration order is the display order for the deck, edit screen and history; `toAnswers`/`toDrafts` are the only positional mapping | `StatusUpdateQuestionTest`, `StoredStatusUpdateTest`, `StatusUpdateHistoryTest` |
...
| `statusupdate/StatusUpdateModels.kt` | `clockOutId` format `locationId@clockOutAtMillis` keys notifications, claims, undo, and the detail/edit route argument; `StatusUpdate` is the domain model (answers keyed by `StatusUpdateQuestion`); persistence is `StoredStatusUpdate`, whose fields are persisted by name, so new fields need defaults and existing ones must not be renamed | `StatusUpdateCoordinatorTest`, `SharedPrefsStatusUpdateLocalDataSourceTest` |
...
| `statusupdate/ui/StatusUpdateCardStack.kt` | stateless; swipe threshold; `StatusUpdateTestTags.CARD`; `CARD_COUNT` is derived from `StatusUpdateQuestion.entries` | `StatusUpdateCardStackTest` (androidTest), `StatusUpdateCardStackUiStateTest` |
